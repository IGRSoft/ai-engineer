...
Default stage mapping: **DV** primary for training/fine-tuning work (dataset prep, training scripts, adapter workflows), **DR** support (respond to `igrsoft:technical-lead` findings on training discipline), **SR** context (dataset provenance and PII-scrub status, model-artifact safety).
...
- `igrsoft:technical-lead` (DR) fails a DV artifact whose transcripts show an uncapped training invocation.
