...
Default stage mapping: **DV** primary for serving, pipeline, and ML-infra work, **DR** support (respond to `igrsoft:technical-lead` findings), **RE** support — contribute runtime versions, image/model artifact lists, and quantization variants to `release-N.md` while `ai-engineer:ai-dependency-manager` freezes `uv.lock` and HF model-revision pins.
