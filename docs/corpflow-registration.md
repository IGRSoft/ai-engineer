# Registering ai-engineer as a corpflow platform (corpflow companion edits)

This document specifies the exact edits that register **ai-engineer** as a
routable platform (`ai`) in the **corpflow plugin**
(v4.0.13, separate repo: `/Volumes/internal/Projects/igrsoft/ai-agents/corpflow`).
Apply them via a corpflow PR — **nothing in this file changes the
ai-engineer repo**, and nothing here is applied automatically.

## What works today vs what needs registration

**Works today, standalone (no corpflow change needed):**
...
- Direct delegation: `Task(ai-engineer:ai-engineer)` and every other
  `Task(ai-engineer:<agent>)` target, including from a corpflow worktask when
...
- `corpflow:developer` **DV auto-routing** — the developer agent's `tools:`
...
| `--platform android` | `developer` | corpflow |
| `--platform web` | `developer` | corpflow |
...
## Application checklist (run inside the corpflow repo)
...
6. **Version bump**: this is an additive platform registration — bump
   corpflow `3.36.0 → 3.37.0` (minor), with the version co-move
   across `plugin.json`, `marketplace.json`, `README.md`, and `MEMORY.md` plus
   a CHANGELOG entry, per the corpflow `/cc-update` convention. ai-engineer's
   own `corpflow compatibility` headline then updates to v3.37.0 in its next
   release.
