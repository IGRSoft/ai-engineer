...
description: Shared AI-stack marker-to-domain-to-agent routing table for the ai-engineer router, igrsoft DV dispatch, and commands. Reference when deciding which ai-engineer agent owns a task, file, or repository.
effort: low
---

# Framework Detection & Agent Routing

Single source of truth for the AI-stack marker → domain → agent mapping used by `ai-engineer:ai-engineer` (router), `igrsoft:developer` DV dispatch, and every ai-engineer command that scopes work per domain. Keep command-local detection logic in sync with this file — do not fork the tables.
