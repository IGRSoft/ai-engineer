...
2. **Base model confirmation** — if Phase 0 step 6 recorded a candidate, carry it forward unchanged. If it found none, recommend one here and mark it "unpinned": name the candidate, record its **parameter count**, and tag it "verify ID and revision against current provider/hub docs — never from memory". This step must resolve a parameter count, because step 4's memory arithmetic is instantiated against it and cannot run on an absent number.
3. **Method selection** — per `skills/finetuning/peft-lora` and `skills/finetuning/preference-tuning`: LoRA (behavior/format/domain from gold outputs) vs QLoRA (same job under the VRAM budget from step 4) vs SFT→DPO (directional "better vs worse" targets — needs a competent SFT baseline first). Include the config starting point for the scenario (r, alpha ≈ 2r, target_modules, dropout). **GRPO/RLVR branch:** when task success is decided by a *program* (unit tests, schema validation, math ground truth, tool-call match), route to `skills/finetuning/grpo-rlvr-training` instead of the preference ladder — DPO for taste, GRPO for reasoning. That branch requires both preconditions stated in the skill (a verifier exists; the base model's success rate is already nonzero) — if either fails, the plan says SFT first and returns here afterwards. Where the traces feeding any of these are already graded, source rows via `skills/finetuning/trace-to-training-data` rather than re-curating raw text.
4. **GPU memory budget** — symbolic formulas per `skills/finetuning/training-optimization` `references/gpu-memory-math.md` (weights + gradients + optimizer states + activations + ≥10–15% headroom), instantiated from the Phase 0 base-model parameter count and **only** against probed or declared hardware; verdict fits / tight / doesn't-fit plus which fit-ladder rungs to plan (accumulation, checkpointing, QLoRA). No hardware known → keep it symbolic and mark "instantiate on the training host".
5. **Hyperparameter starting points** — LR (~2e-4 LoRA-class, sweep), cosine schedule + warmup, effective batch = micro × accumulation (retune LR when it changes), fixed seed; DPO: beta ~0.1 swept against held-out win rate, never training loss. All labeled sweep origins; TRL/PEFT arg names to be verified via context7 (Rule 6).
6. **Eval plan + success criteria** — per `skills/evals/eval-design`: baseline run on the pinned eval set **before** training (temperature 0, eval-set version recorded); target metrics by task type with explicit pass thresholds; a **general-capability regression slice** that must not regress (the catastrophic-forgetting detector); gate wiring per `skills/evals/regression-gates`; subjective quality via `skills/evals/llm-judge`.
7. **Launch plan** — two runs, both as printed commands: **smoke** (capped `max_steps`, ~256-record subsample, fixed seed; duration class: minutes; success = falling loss, no NaN, sane trainable-param %, peak memory recorded vs estimate) then **full** (command + dataset version + CUDA host; duration class + cost drivers: GPU-hours ≈ steps × sec/step, spend scales with params, sequence length, epochs — no absolute prices, Rule 4).
8. **Promotion gate** — per `skills/finetuning/checkpoint-promotion`: name the frozen baseline and the version-pinned capability-drift suite the checkpoint will be re-scored against, and state the drift budget (≤1pt noise · 2–5pt seed-variation rerun · >5pt hard fail, which no target-task gain buys back). Every reported margin carries its half-width; a margin inside its own interval resolves `REJECT (uncertain)`. The plan must say what happens on `REJECT` — the escalation ladder's first rung, not a menu — because a plan with no reject path silently assumes success.
9. **Export step** — per `skills/finetuning/quantized-export`, planned only as the `PROMOTE` branch: merged vs LoRA-only (LoRA-only pins the base repo *and* revision), the target runtime and format, and the mandatory pre/post smoke test over 3–5 goldens. Note that pre-export generations must be captured *before* the export runs — a plan that omits this loses the only baseline the smoke test can diff against.
...
- `skills/finetuning/grpo-rlvr-training` — the GRPO/RLVR branch of step 3: verifiability preconditions, reward-function design and its inspection gate, variant selection
- `skills/finetuning/trace-to-training-data` — sourcing step 1's rows from already-graded traces (rejection sampling, pairs from passing/failing runs) instead of raw text
- `skills/finetuning/checkpoint-promotion` (+ `references/gate-templates.md`) — step 8's four-stage gate, drift budget, and terminal PROMOTE/REJECT contract
- `skills/finetuning/quantized-export` (+ `references/export-commands.md`) — step 9's merge/format decision and the pre/post smoke test
