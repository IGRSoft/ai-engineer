...
# Compile-only gate (what igrsoft's DR stage calls)
...
| `--no-test` | off | Sync and import-check only; skip the test phase. **This is the DR compile-only gate** — `igrsoft:developer` depends on this flag existing. |
