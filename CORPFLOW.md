...
This is the **only** file in ai-engineer that knows corpflow exists. Delete it and the plugin is
...
`.context/state.json` exists → yes. Otherwise every rule below is inert and ai-engineer behaves exactly
...
### Stages owned by ai-engineer

ai-engineer owns these stages when dispatched:
...
Build and test **only** through `/ai-engineer:build-test`. Do not invoke the toolchain
...
```markdown
## <STAGE> Summary — ai-engineer:<agent>
**Verdict**: pass | fail | needs_changes
...
```yaml
handoff:
  from: "ai-engineer:ai-architector"
  to: "corpflow:software-architector"
  stage: "AR"
  run_index: <N>
  status: "completed"
  verdict: "pass"
  artifacts: [".context/ai-architecture.md"]
  metadata:
    patterns_selected: []
    constraints: []
```
...
```yaml
handoff:
  from: "ai-engineer:<support-agent>"
  to: "ai-engineer:llm-engineer"
  stage: "DV"
  run_index: <N>
  status: "completed"
  verdict: "pass"
...
When a corpflow contract changes, this file changes and nothing else in ai-engineer does. That property
...
- Hooks that describe orchestrator interop say "the orchestrator" generically. They work under
  ...
  be coupled.
- ai-engineer's own version does not track corpflow's. Record the targeted corpflow version below and
