...
- Build and test **only** through `/ai-engineer:build-test`; never invoke the toolchain directly.
...
| Stage | Delta | Verdict |
|---|---|---|
| AR | `key_decisions` + `open_questions` anchored in `ai-architecture.md`; no `files_touched` / `worktree` / `architecture` | ok / blocked / escalate |
