#!/usr/bin/env bash
# SubagentStop → audit.jsonl writer (ai-engineer plugin, v1.0.0+).
# Emits a `subagent_stopped` row with actor "ai-engineer:hook:audit-subagent".
#
# ADVISORY ROW: ai-engineer specialists run as subagents under an orchestrating plugin,
...
read_stdin() {
  if [ "$SELF_TEST" -eq 1 ]; then
    printf '%s' '{"agent_type":"ai-engineer:llm-engineer","agent_id":"agt_test","session_id":"sess_test","duration_ms":12345,"parent_agent_id":"agt_parent"}'
  else
    cat
  fi
}
...
ROW=$(printf '%s' "$PAYLOAD" | jq -c \
  ...
  {
    ts: $ts,
    actor: "ai-engineer:hook:audit-subagent",
...
if [ "$SELF_TEST" -eq 1 ]; then
  printf '%s\n' "$ROW" | jq -e '
    ...
    and .subject == "ai-engineer:llm-engineer"
    ...
    and .actor == "ai-engineer:hook:audit-subagent"
  ' >/dev/null \
